...
(function () {
	var oldScrollTop = 0,
	    ...
	    lowFpsCount = 0,
	    stupidWordContainer = void 0,
	    currentStupidWord = void 0;
	...
	if (location.hash.length > 2 && location.hash.indexOf('favs=') !== -1) {
		...
		favs.split('|').forEach(function (id) {
			jsmain__cookieClicker.create('fav-' + id, true, 100);
		});
	}
	...
	jsmain___ajax('https://api.trondheimdc.no/events/tdc2017/sessions', function (data) {
		...
		if (window.location.href.indexOf('workshops') !== -1) {
			...
			var speakers = sessions.concat(fakeSpeakers).reduce(function (acc, curr) {
	...
	function loadProgram(sessions) {
		...
		slotTimes.forEach(function (time) {
			if (!slots[time]) {
				...
				var type = isLunch ? 'lunch' : 'refreshments';
				var extra = isLunch ? ' title="<b>BartJS Podcast Live</b>"' : '';
			...
			slots[time].forEach(function (sesh) {
				...
				if (sesh.format === 'fake') {
					...
					var id = time + '-' + rom,
					    fav = jsmain__cookieClicker.read("fav-" + id) === 'true',
		...
		document.addEventListener('click', function (e) {
			...
			if (that) {
				...
				jsmain___ajax(url, function (data) {
					...
					var speaker = sesh.foredragsholdere[0];
					...
					setTimeout(function () {
						stupidWordContainer = jsmain___si('.speakerCard figcaption span');
						writeStupidWord(0);
					}, 6000);
				});
			}
		...
		document.addEventListener('click', function (e) {
			...
			if (that) {
				...
				jsmain__cookieClicker.create("fav-" + id, sesh.classList.contains('is-fav'), 100);

				if (inModal) {
					jsmain___si('.sesh[data-id="' + id + '"]').classList.toggle('is-fav');
				}
			}
		});

		jsmain___si('.exp-fav').addEventListener('click', function (e) {
			e.preventDefault();
